...
void INA_read(void *parameter) {
  ...
  Voltage_string = String(ina.getVoltage(), 3);
  Voltage_string.toCharArray(Voltage_char,8);
  Serial.println(Voltage_string);
  mqttClient.publish("Oleg_N/Home_01/Voltage", 0, true, Voltage_char);

  // Читаем ток
  Serial.print(F("Current: "));
  Current_string = String(ina.getCurrent(), 3);
  Current_string.toCharArray(Current_char,8);
  Serial.println(Current_string);
  mqttClient.publish("Oleg_N/Home_01/Current", 0, true, Current_char);

  // Читаем мощность
  Serial.print(F("Power: "));
  Power_string = String(ina.getPower(), 3);
  Power_string.toCharArray(Power_char,8);
  Serial.println(Power_string);
  mqttClient.publish("Oleg_N/Home_01/Power", 0, true, Power_char);
